...
class SummaryClassificationSyntheticWithCV:
    ...
    def train_and_evaluate_models(self):
        X = np.stack(self.df_embeddings['embeddings'].apply(lambda x: np.array(x).flatten()))
        y = self.df_embeddings['label'].to_numpy()
        ...
        for train_index, test_index in skf.split(X, y):
            ...
            for name, Classifier in self.classifiers.items():
                ...
                results[name].append(f1_score)
        for name in results:
            self.f1_scores[name] = np.mean(results[name])
    ...
    def determine_winner(self):
        if self.f1_scores:
            ...
            logging.info("No F1 scores available to determine a winner.")
